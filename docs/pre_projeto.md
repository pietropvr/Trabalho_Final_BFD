...
![Wireframe Desktop - Dashboard](assets/wireframe-dashboard.png) Wireframe mostrando um menu lateral fixo usando CSS Grid e a lista de pets em formato de cartões na área principal.

Tela de Celular (Detalhe do Pet):
![Wireframe Mobile - Detalhe do Pet](assets/wireframe-mobile.png) Wireframe mostrando os cartões empilhados em uma coluna única (Media Queries) para leitura responsiva, com botão flutuante de "Adicionar Registro".
...
Risco Técnico Principal: A geração do arquivo standalone HTML/JS exige embutir informações dinâmicas diretamente nas tags <style> e <script> via Jinja2, sem quebrar a sintaxe. Mitigação: Criaremos um template específico e isolado só para isso, testando sua renderização desde a primeira semana.
